...
const downloadUrl = 'https://veoveo.dripdev.dev/descargar';
...
async function main() {
  ...
  const recipients = snap.docs
    ...
    .filter(
      (user) =>
        typeof user.pushToken === 'string' && user.pushToken.startsWith('ExponentPushToken'),
    )
  ...
  const messages = recipients.map((user) => ({
    ...
    body: `VeoVeo ${targetVersion} ya esta lista. Descargala para disfrutar de las mejoras nuevas.`,
    data: {
      type: 'app_update',
  ...
  for (let i = 0; i < messages.length; i += 100) {
    const batchTickets = await enviarBatch(messages.slice(i, i + 100));
    ...
    JSON.stringify(
      {
        ...
        acceptedTickets: accepted,
        failedTickets: failed.length,
        failedDetails: failed.map((ticket) => ({
