...
export function BibliotecaTab({ fontFamily, refreshToken = 0, onPeliculaClick, onPerfilClick, userFoto, resetToken = 0 }: Props) {
    ...
    filtered.forEach((p: any) => {
      if (seccion === 1 && orden === 'recientes') {
        ...
        if (header !== lastMonthHeader) {
          ...
          result.push({ isHeader: true, title: monthYear === currentMonthYear ? 'Vistas este mes' : header });
  ...
  return (
    <View style={styles.flex}>
      <View style={[styles.tabsHeaderOverlay, { height: Math.max(insets.top, 12) + 130 + (buscar ? 60 : 0) }]}>
      ...
      <View style={[styles.headerRow, { top: Math.max(insets.top, 12) + 12 }]}>
        <Text style={[styles.titulo, { fontFamily, flex: 1 }]} numberOfLines={1}>{seccion === 0 ? t('por_ver') : t('vistas')}</Text>
...
const styles = StyleSheet.create({
  ...
  headerRow: { position: 'absolute', left: 24, right: 24, zIndex: 100, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  actionsTopRow: { flexDirection: 'row', gap: 8, alignItems: 'center' },
  perfilBtnMini: { marginLeft: 4 },
  perfilInnerMini: { width: 46, height: 46, borderRadius: 23, borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.4)', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  perfilFotoMini: { width: '100%', height: '100%' },
  iconBtn: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' },
  tabs: { position: 'absolute', alignSelf: 'center', left: 0, right: 0, flexDirection: 'row', justifyContent: 'center', gap: 24, zIndex: 100 },
  tab: { fontSize: 18, color: 'rgba(255,255,255,0.4)', fontWeight: '600' },
  tabActivo: { color: '#fff', fontWeight: '800' },
  searchField: { position: 'absolute', left: 20, right: 20, zIndex: 110, borderRadius: 24, backgroundColor: CardSurface, paddingHorizontal: 20, height: 50, color: '#fff', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  tabsHeaderOverlay: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 90, overflow: 'hidden' },
  ...
  dot: { position: 'absolute', top: 8, right: 8, width: 10, height: 10, borderRadius: 5, borderWidth: 1.5, borderColor: 'rgba(255,255,255,0.2)', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.5, shadowRadius: 2, elevation: 3 },
  monthHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, marginVertical: 20, paddingHorizontal: 4 },
  monthTitle: { color: 'rgba(255,255,255,0.5)', fontSize: 12, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 1 },
