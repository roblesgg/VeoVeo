...
import {
  ...
  RefreshControl, StyleSheet, Text, TextInput, View, Keyboard
} from 'react-native';
...
export function DiscoverTab({ fontFamily, estaActiva, onPeliculaClick, onActorClick, onPerfilClick, userFoto, resetToken = 0 }: Props) {
  ...
  return (
    <View style={styles.flex}>
      ...
      {buscarAtiva && (
        <Animated.View entering={FadeInDown} style={[styles.searchOverlay, { top: Math.max(insets.top, 12) + 80 }]}>
              ...
              renderItem={({ item }) => (
                tipoBusqueda === 'movie' ? (
                  <View style={styles.busquedaPosterGrid}>
                    ...
                    >
                      {item.poster_path ? (
                        <Image source={{ uri: posterUrl(item.poster_path, 'w342')! }} style={styles.fullPoster} />
                      ) : (
...
const styles = StyleSheet.create({
  ...
  fallbackText: { color: 'rgba(255,255,255,0.3)', fontSize: 10, textAlign: 'center' },
  searchBadge: {
    position: 'absolute',
    top: 6,
    right: 6,
  ...
  searchBadgeText: { color: '#fff', fontSize: 10, fontWeight: '800' },
  actorCard: { flex: 1/2, margin: 6, backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 20, padding: 12, alignItems: 'center' },
  actorImgWrapper: { width: 80, height: 80, borderRadius: 40, overflow: 'hidden', marginBottom: 10 },
